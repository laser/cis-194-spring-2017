...
spec = do
  ...
  describe "build" $ do
    it "builds a MessageTree from a list of LogMessages" $ do
      ...
      -- My function, using foldr, does create a binary search tree, but it is of a different shape
      build [foo, baz, bif] `shouldBe` Node (Node Leaf baz (Node Leaf foo Leaf)) bif Leaf
